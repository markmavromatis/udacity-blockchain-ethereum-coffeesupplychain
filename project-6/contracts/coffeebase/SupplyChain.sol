...
contract SupplyChain is ConsumerRole, DistributorRole, FarmerRole, RetailerRole, Ownable {
  ...
  modifier checkValue(uint _upc) {
    ...
    address payable consumer = items[_upc].consumerID;
  ...
  function kill() public {
    address payable currentOwner = address(uint160(owner()));
  ...
  function buyItem(uint _upc) forSale(_upc) paidEnough(items[_upc].productPrice) checkValue(upc) public payable
  {
    
    Item memory foundItem = items[_upc];
    require(foundItem.originFarmerID != address(0));

    // Update the appropriate fields - ownerID, distributorID, itemState
    foundItem.itemState = State.Sold;
    foundItem.ownerID = msg.sender;
    foundItem.distributorID = msg.sender;
    
    // Transfer money to farmer
    uint price = foundItem.productPrice;
    foundItem.originFarmerID.transfer(price);
    
    items[_upc] = foundItem;
  ...
  ) 
  {
    ...
    productNotes = foundItem.productNotes;
    productPrice = foundItem.productPrice;
    itemState = uint(foundItem.itemState);
    distributorID = foundItem.distributorID;
    retailerID = foundItem.retailerID;
    consumerID = foundItem.consumerID;
